Shader "Custom/TextureBlendByVertexColor"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandardSpecular o)
        {
            ...
            /*
            float color0Lerp = clamp(IN.color.r + IN.color.g + IN.color.b, 0, 1);
        
            color1 = clamp(color1 * IN.color.r, 0, 1);
            color2 = clamp(color2 * IN.color.g, 0, 1);
            color3 = clamp(color3 * IN.color.b, 0, 1);

            fixed4 finalColor = clamp(color1 + color2 + color3, 0, 1);

            finalColor = lerp(color0, finalColor, color0Lerp);*/
            
            fixed4 finalColor = lerp(lerp(lerp(color0, color1, IN.color.r), color2, IN.color.g), color3, IN.color.b);
